...
class CheckerAgent:
    ...
    def __init__(self, model_name: str = "llama3:latest", prompt_filename: str = "checker_prompt.txt", drivers_path: str = "config/drivers.json"):
        ...
        # ۱. بارگذاری شناسه پیشران‌های معتبر برای ممیزی قطعی پایتون (Zero-Latency Guard)
        ...
        # حل پویا و ایمن مسیر فایل پرامپت ممیز برای جلوگیری از خطای FileNotFoundError در محیط Streamlit
        ...
        # در صورت عدم یافتن فایل، یک پرامپت پشتیبان به عنوان مرز دفاعی ثانویه قرار داده می‌شود
...
"""
    ...
    def _validate_hard_constraints(self, original_proposal: str, data: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        for idx, item in enumerate(alignments):
            ...
            # ۱. بررسی تطابق شناسه پیشران با پیشران‌های معتبر تافکو (بررسی قطعی پایتون - مانع اصلی چالش فعلی)
            if self.valid_driver_ids and driver_id not in self.valid_driver_ids:
                ...
                return False, f"شناسه پیشران نامعتبر است: '{driver_id}'. شما باید شناسه را دقیقاً از لیست پیشران‌های استراتژیک بخش ۱ کپی کنید (مانند {example_id}). به هیچ وجه عنوان پروپوزال یا عنوان طرح را در این فیلد قرار ندهید."
